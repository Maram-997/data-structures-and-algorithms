...
const screenForNames = (arr) => {
  ...
  let rgx3 = /^(Dr.\s)[A-Z]/;
  arr.map(element => {
    if (rgx.test(element) || rgx1.test(element) || rgx2.test(element) || rgx3.test(element)) {
      newArr.push(element)
    }
  })
  return newArr;
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let newArr = [];
  let compareValue = starWarsData[0].mass
  arr.filter(element => {
    if (parseInt(element.mass) > parseInt(compareValue)) {
